...
def create_attorney_meeting(
    ...
    _org_email = os.environ.get("ORG_EMAIL", "")
    _center_email = os.environ.get("ORG_CENTER_EMAIL", "")
    core_attendees = [e for e in [_center_email, _org_email] if e]
...
if __name__ == "__main__":
    # Criar reunião para Felix Mendoza
    # Data: 04/02/2026 às 12:00 PM EST (11:00 AM CT)

    meeting_date = datetime(2026, 2, 4, 12, 0, 0)  # 12:00 PM EST

    # ⚠️ NUNCA incluir cliente nos participantes do Calendar!
    # Cliente recebe link do Meet por email separado
    # TODO: Configure attendees via settings or DB
    attendees = [
        # Add org emails and team members as needed
    ]

    description = """Reunião sobre RFE - Felix Mendoza (EB-1A)

Case #: 33
Receipt No: IOE0934666966
Paralegal: Juliana Moreschi
Team: Vermelho

Notas:
- Michelle (esposa) participará da reunião
- Preparar documentação do RFE antes da reunião
- Revisar timeline do caso"""

    print("\n" + "="*60)
    print("📅 CRIANDO REUNIÃO COM ATTORNEY - FELIX MENDOZA")
    print("="*60)

    result = create_attorney_meeting(
        client_name="Felix Mendoza",
    ...
    if result and result.get("success"):
        ...
        print("\n⚠️ AÇÃO PENDENTE:")
        print("   - Obter email da Michelle para adicionar ao convite")
        print("   - Juliana deve preparar material do RFE")
