...
// ===========================================
      ...
      function restoreSection() {
        ...
        const validSections = ['dashboard', 'lor', 'ps', 'package', 'pdf', 'clients', 'leads', 'docs', 'tasks', 'agenda'];
      ...
      // Override showSection to load clients/leads when navigating
      ...
      // Tab switching
      function showLeadsTab(tab) {
        ['list', 'pipeline', 'metrics'].forEach(t => {
          const btn = document.getElementById('leads-tab-btn-' + t);
          const content = document.getElementById('leads-' + t + '-tab');
      ...
      function sortLeads(field) {
        if (leadsSortBy === field) {
          leadsSortOrder = leadsSortOrder === 'asc' ? 'desc' : 'asc';
        } else {
          leadsSortBy = field;
          leadsSortOrder = field === 'lead_score' ? 'desc' : 'asc';
        }
        loadLeadsList();
      }

      // Load leads list from API
      async function loadLeadsList() {
        ...
        const leadStatus = document.getElementById('leads-filter-lead-status').value;
        ...
        try {
          ...
          loadingEl.innerHTML = '<p style="color: var(--error, #e74c3c);">Error loading leads. Check console.</p>';
        }
      }
      ...
      function renderLeadsTable() {
        ...
        tbody.innerHTML = leadsData.map(lead => renderLeadRow(lead)).join('');
      }

      // Render a single lead row
      function renderLeadRow(lead) {
        const score = lead.lead_score || 0;
        const scoreClass = score >= 75 ? 'score-hot' : score >= 50 ? 'score-qualified' : score >= 25 ? 'score-warm' : 'score-cold';
        const source = lead.source || 'WPP';
        const stage = lead.pipeline_stage || 'NEW_LEAD';
        const name = escapeHtml(lead.name || lead.whatsapp_name || 'Unknown');
        const phone = escapeHtml(lead.phone || '-');
        const interest = escapeHtml(lead.visa_interest || '-');
        const date = lead.created_at ? new Date(lead.created_at).toLocaleDateString('pt-BR') : '-';

        return '<tr onclick="openLeadDetail(\'' + lead.id + '\')">' +
          '<td><span class="score-badge ' + scoreClass + '">' + score + '</span></td>' +
          '<td style="font-weight: 500;">' + name + '</td>' +
          '<td style="font-family: monospace; font-size: 0.8rem;">' + phone + '</td>' +
          '<td><span class="source-badge source-' + source + '">' + source + '</span></td>' +
          '<td><span class="stage-badge stage-' + stage + '">' + (STAGE_LABELS[stage] || stage) + '</span></td>' +
          '<td>' + interest + '</td>' +
          '<td style="font-size: 0.8rem; color: var(--text-secondary);">' + date + '</td>' +
          '<td style="text-align: center;">' +
            '<button class="leads-action-btn delete" onclick="event.stopPropagation(); deleteLead(\'' + lead.id + '\')" title="Delete">' +
      ...
      async function updateLeadsStats() {
        try {
          ...
          if (response.ok) {
            ...
            document.getElementById('leads-stat-total').textContent = m.total || 0;
            document.getElementById('leads-stat-hot').textContent = (m.by_lead_status || {}).hot || 0;
            document.getElementById('leads-stat-qualified').textContent = (m.by_lead_status || {}).qualified || 0;
            document.getElementById('leads-stat-warm').textContent = (m.by_lead_status || {}).warm || 0;
            document.getElementById('leads-stat-cold').textContent = (m.by_lead_status || {}).cold || 0;
            document.getElementById('leads-stat-today').textContent = m.today || 0;
          }
        } catch (e) {
      ...
      function updateLeadsPagination() {
        const paginationEl = document.getElementById('leads-pagination');
        const totalPages = Math.ceil(leadsTotal / leadsPerPage);
        ...
        document.getElementById('leads-pagination-info').textContent =
          'Page ' + leadsCurrentPage + ' of ' + totalPages + ' (' + leadsTotal + ' leads)';
        document.getElementById('leads-prev-btn').disabled = leadsCurrentPage <= 1;
        document.getElementById('leads-next-btn').disabled = leadsCurrentPage >= totalPages;
      }

      function leadsChangePage(delta) {
        leadsCurrentPage += delta;
        loadLeadsList();
      }
      ...
      function openLeadDetail(leadId) {
        ...
        document.getElementById('lead-source').value = lead.source || 'WPP';
        document.getElementById('lead-score').value = lead.lead_score || 0;
        document.getElementById('lead-stage').value = lead.pipeline_stage || 'NEW_LEAD';
        document.getElementById('lead-visa-interest').value = lead.visa_interest || '';
        document.getElementById('lead-profession').value = lead.profession || '';
        document.getElementById('lead-language').value = lead.language || 'en';
        document.getElementById('lead-status').value = lead.status || 'new';
        document.getElementById('lead-notes').value = lead.notes || '';

        // Read-only info
        const readonlyEl = document.getElementById('lead-readonly-info');
        readonlyEl.style.display = 'block';
        document.getElementById('lead-info-moskit').textContent = lead.moskit_contact_id || '-';
        document.getElementById('lead-info-messages').textContent = lead.message_count || 0;
        document.getElementById('lead-info-created').textContent = lead.created_at ? new Date(lead.created_at).toLocaleString('pt-BR') : '-';
        document.getElementById('lead-info-updated').textContent = lead.updated_at ? new Date(lead.updated_at).toLocaleString('pt-BR') : '-';
        document.getElementById('lead-info-first-contact').textContent = lead.first_contact_at ? new Date(lead.first_contact_at).toLocaleString('pt-BR') : '-';
        document.getElementById('lead-info-pathway').textContent = lead.intake_form_primary_pathway || '-';
      ...
      function showAddLeadModal() {
        ...
        document.getElementById('lead-stage').value = 'NEW_LEAD';
        document.getElementById('lead-language').value = 'en';
        document.getElementById('lead-status').value = 'new';
        document.getElementById('lead-readonly-info').style.display = 'none';
        document.getElementById('lead-modal').style.display = 'flex';
      }
      ...
      async function saveLead(event) {
        ...
        const payload = {
          ...
          lead_score: parseInt(document.getElementById('lead-score').value) || 0,
          pipeline_stage: document.getElementById('lead-stage').value,
          visa_interest: document.getElementById('lead-visa-interest').value,
          profession: document.getElementById('lead-profession').value,
      ...
      // Sync from Moskit
      async function syncLeadsMoskit() {
        showToast('Syncing from Moskit...', 'info');
        try {
          const token = localStorage.getItem('ilc_access_token');
          const response = await fetch('/api/leads/sync/moskit', {
            method: 'POST',
            headers: { 'Authorization': 'Bearer ' + token }
          });
          if (response.ok) {
            ...
            leadsLoaded = false;
            loadLeadsList();
            showToast('Moskit sync: ' + (data.imported || 0) + ' new, ' + (data.updated || 0) + ' updated', 'success');
          } else {
      ...
      async function loadKanban() {
        // Fetch all leads (no pagination for kanban)
        try {
          const token = localStorage.getItem('ilc_access_token');
          const response = await fetch('/api/leads?per_page=500&sort_by=lead_score&sort_order=desc', {
            headers: token ? { 'Authorization': 'Bearer ' + token } : {}
          });
          ...
          const stages = ['NEW_LEAD', 'LEAD_QUALIFICATION', 'INTAKE_CALL', 'CONSULTATION', 'CLOSING', 'VISA_IN_PROGRESS'];
          stages.forEach(stage => {
            const container = document.getElementById('kanban-cards-' + stage);
            const countEl = document.getElementById('kanban-count-' + stage);
            const stageLeads = allLeads.filter(l => (l.pipeline_stage || 'NEW_LEAD') === stage);
            ...
            container.innerHTML = stageLeads.map(lead => {
              ...
              return '<div class="kanban-card" draggable="true" ondragstart="dragLead(event, \'' + lead.id + '\')" onclick="openLeadDetailFromKanban(\'' + lead.id + '\')" data-lead-id="' + lead.id + '">' +
                '<div class="kanban-card-name">' + name + '</div>' +
                '<div class="kanban-card-meta">' +
                  '<span class="score-badge ' + scoreClass + '" style="width: 30px; height: 20px; font-size: 0.7rem;">' + score + '</span>' +
                  '<span class="source-badge source-' + source + '" style="font-size: 0.7rem;">' + source + '</span>' +
                  '<span>' + date + '</span>' +
                '</div>' +
              '</div>';
            }).join('');
      ...
      // Kanban data cache for opening detail from kanban
      async function openLeadDetailFromKanban(leadId) {
        try {
          const token = localStorage.getItem('ilc_access_token');
          const response = await fetch('/api/leads/' + leadId, {
            headers: token ? { 'Authorization': 'Bearer ' + token } : {}
          ...
          if (response.ok) {
            ...
            // Temporarily push into leadsData so openLeadDetail works
            if (!leadsData.find(l => l.id === lead.id)) {
              leadsData.push(lead);
            }
      ...
      async function dropLead(event, newStage) {
        ...
        try {
          ...
          if (response.ok) {
            loadKanban();
            showToast('Lead moved to ' + (STAGE_LABELS[newStage] || newStage), 'success');
          } else {
      ...
      async function loadMetrics() {
        const loadingEl = document.getElementById('leads-metrics-loading');
        const contentEl = document.getElementById('leads-metrics-content');
        loadingEl.style.display = 'block';
        contentEl.style.display = 'none';

        try {
          const token = localStorage.getItem('ilc_access_token');
          const [metricsRes, pipelineRes, trendsRes] = await Promise.all([
            fetch('/api/leads/metrics', { headers: token ? { 'Authorization': 'Bearer ' + token } : {} }),
            fetch('/api/leads/metrics/pipeline', { headers: token ? { 'Authorization': 'Bearer ' + token } : {} }),
            fetch('/api/leads/metrics/trends', { headers: token ? { 'Authorization': 'Bearer ' + token } : {} })
          ]);

          if (!metricsRes.ok) throw new Error('Failed to load metrics');

          const metricsData = await metricsRes.json();
          const pipelineData = pipelineRes.ok ? await pipelineRes.json() : {};
          const trendsData = trendsRes.ok ? await trendsRes.json() : {};

          const m = metricsData;

          // Top stats
          document.getElementById('metric-total-leads').textContent = m.total || 0;
          document.getElementById('metric-avg-score').textContent = (m.avg_score || 0).toFixed(1);

          const converted = (m.by_status || {}).converted || 0;
          const totalLeads = m.total || 1;
          document.getElementById('metric-conversion-rate').textContent = ((converted / totalLeads) * 100).toFixed(1) + '%';
          document.getElementById('metric-this-month').textContent = m.this_month || 0;
          ...
          renderSourceChart(m.by_source || {});
          renderPipelineFunnel(pipelineData.pipeline || {});
          renderScoreDistribution(m.by_lead_status || {});
          renderTrendsChart(trendsData);

        } catch (error) {
          console.error('Metrics error:', error);
          loadingEl.innerHTML = '<p style="color: var(--error, #e74c3c);">Error loading metrics.</p>';
        }
      }
      ...
      function renderSourceChart(sourceData) {
        const ctx = document.getElementById('chart-leads-source');
        if (chartInstances.source) chartInstances.source.destroy();

        const labels = Object.keys(sourceData).map(k => SOURCE_LABELS[k] || k);
      ...
      function renderPipelineFunnel(pipelineData) {
        const ctx = document.getElementById('chart-pipeline-funnel');
        if (chartInstances.pipeline) chartInstances.pipeline.destroy();

        const stageOrder = ['NEW_LEAD', 'LEAD_QUALIFICATION', 'INTAKE_CALL', 'CONSULTATION', 'CLOSING', 'VISA_IN_PROGRESS'];
        const labels = stageOrder.map(s => STAGE_LABELS[s] || s);
        const values = stageOrder.map(s => {
          const stage = pipelineData[s];
          return stage ? (stage.count || 0) : 0;
        });
        ...
        chartInstances.pipeline = new Chart(ctx, {
          ...
          options: {
            ...
            scales: {
              x: { ticks: { color: '#9ca3af' }, grid: { color: '#333' } },
      ...
      function renderScoreDistribution(statusData) {
        const ctx = document.getElementById('chart-score-dist');
        if (chartInstances.scoreDist) chartInstances.scoreDist.destroy();

        const labels = ['Cold (<25)', 'Warm (25-49)', 'Qualified (50-74)', 'Hot (75+)'];
        const values = [statusData.cold || 0, statusData.warm || 0, statusData.qualified || 0, statusData.hot || 0];
      ...
      function renderTrendsChart(trendsData) {
        const ctx = document.getElementById('chart-leads-trend');
        if (chartInstances.trends) chartInstances.trends.destroy();

        const dailyArray = trendsData.daily || [];
        const labels = dailyArray.map(d => {
          const parts = d.date.split('-');
          return parts[2] + '/' + parts[1];
        });
        const values = dailyArray.map(d => d.count || 0);
        ...
        chartInstances.trends = new Chart(ctx, {
          ...
          options: {
            ...
            scales: {
              ...
              y: { ticks: { color: '#9ca3af', stepSize: 1 }, grid: { color: '#333' }, beginAtZero: true }
            }
          }
        });
      }
...
// Cookie helper functions
        ...
        async function loadTasks() {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    renderTasksTable();
                    // Also render kanban if in kanban mode
        ...
        function renderKanbanBoard() {
            ...
            const statusGroups = {};
            Object.keys(kanbanStatusConfig).forEach(status => {
                statusGroups[status] = [];
            });
            ...
            tasksData.forEach(task => {
                ...
                if (statusGroups[status]) {
                    ...
                    statusGroups['A fazer'].push(task);
                }
            });

            // Build HTML
            let html = '';
            Object.entries(kanbanStatusConfig).forEach(([status, config]) => {
                const tasks = statusGroups[status] || [];
                html += `
                    <div class="kanban-column" data-status="${status}">
                        <div class="kanban-column-header" style="background: ${config.bg};">
                        ...
                        <div class="kanban-column-cards" data-status="${status}">
                            ${tasks.length > 0
                                ...
                                : `<div class="kanban-column-empty"><span>${config.emoji}</span>Nenhuma tarefa</div>`
                            }
                        </div>
                    </div>
                `;
        ...
        function renderKanbanCard(task) {
            const responsavelNames = getMultiRelationNames('Responsável', task.responsavel?.ids) || '-';
            const clienteNames = getRelationName('Cliente', task.cliente?.ids) || '-';
            const prioridade = task.prioridade || 'Media';
            const priorityConfig = priorityColors[prioridade] || { bg: '#2d2d2d', text: '#9b9b9b' };
            ...
            if (task.deadline) {
                ...
                deadlineHtml = `<span class="deadline ${deadlineClass}">📅 ${formatDate(task.deadline)}</span>`;
            }
                ...
                <div class="kanban-card" data-task-id="${task.id}" draggable="true">
                    <div class="kanban-card-header">
                        <div class="kanban-card-title">
                            <a href="${task.url || '#'}" target="_blank" title="${escapeHtml(task.tarefa)}">${escapeHtml(task.tarefa)}</a>
                        </div>
                        <button class="kanban-card-delete" onclick="deleteTask('${task.id}', event)" title="Excluir">✕</button>
                    </div>
                    <div class="kanban-card-meta">
                        <span class="badge responsavel">${responsavelNames}</span>
                        ${clienteNames !== '-' ? `<span class="badge cliente">${clienteNames}</span>` : ''}
                    </div>
                    <div class="kanban-card-footer">
                        <span class="priority-badge" style="background: ${priorityConfig.bg}; color: ${priorityConfig.text};">
                            ${prioridade}
                        </span>
                        ${deadlineHtml}
                    </div>
        ...
        async function handleDrop(e) {
            ...
            // Move card visually first (optimistic UI)
            const emptyPlaceholder = e.currentTarget.querySelector('.kanban-column-empty');
            if (emptyPlaceholder) {
            ...
            try {
                ...
                draggedCard.classList.add('save-success');
                setTimeout(() => draggedCard.classList.remove('save-success'), 500);
        ...
        window.showSection = function(section, updateHash = true) {
            ...
            if (section === 'tasks') {
                if (!notionSchema) {
                    loadNotionSchema().then(() => {
                        loadTasks();
                        initTasksView(); // Initialize view mode (list/kanban)
                    });
                } else {
                    loadTasks();
                    initTasksView(); // Initialize view mode (list/kanban)
