...
def image_to_pdf(image_path: str, output_path: str) -> str:
    ...
    try:
        ...
        raise ValueError(f"Failed to convert image '{os.path.basename(image_path)}': {e}")
...
def get_pdf_page_count(pdf_path: str) -> int:
    ...
    try:
        ...
        logger.warning(f"Could not read page count for '{os.path.basename(pdf_path)}': {e}")
        return 1  # Fallback: assume 1 page
    ...
    for pdf_path in pdf_paths:
        if os.path.exists(pdf_path):
            try:
                ...
                logger.warning(f"Could not merge '{os.path.basename(pdf_path)}': {e}")

    if added_count == 0:
        merger.close()
        raise ValueError("No valid PDF files to merge. Check that uploaded files are valid PDFs.")
...
class PackageBuilder:
    ...
    def add_document(
        ...
        if exhibit not in EXHIBITS:
            logger.warning(f"Invalid exhibit: {exhibit}")
            return False
        ...
        if ext in [".jpg", ".jpeg", ".png", ".gif", ".webp", ".tiff"]:
            # Convert image to PDF with unique name to avoid conflicts
            unique_id = uuid.uuid4().hex[:8]
            pdf_path = os.path.join(
                self.output_dir,
                f"converted_{unique_id}_{Path(filepath).stem}.pdf"
            )
            filepath = image_to_pdf(filepath, pdf_path)
            logger.info(f"Converted image to PDF: {pdf_path}")

        elif ext in [".docx", ".doc"]:
            logger.warning(f"DOCX not supported: {filepath}")
            return False
    ...
    def build(
        ...
        logger.info(f"Package built: {output_path}")
        logger.info(f"Total exhibits: {sum(1 for d in self.documents.values() if d)}")
        logger.info(f"Total documents: {sum(len(d) for d in self.documents.values())}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="CaseHub Package Builder")
