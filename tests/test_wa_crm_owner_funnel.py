...
def db():
    ...
    mkuser(10, 1, "UsuarioDemo Escritorio Demo", "#7c3aed")   # explicit purple
    ...
    v2 = contact(2, 1, "+552", "PessoaDemo", None, None, "reuniao")
...
def test_name_priority(db):
    ...
    assert r["+552"]["name"] == "PessoaDemo"        # display_name fallback
    assert r["+553"]["name"] == "+553"         # phone fallback


def test_owner_badge_shape(db):
    r = _rows(db)
    assert r["+551"]["owner"] == {"user_id": 10, "name": "UsuarioDemo Escritorio Demo", "color": "#7c3aed"}
...
def test_resolve_owner_tenant_isolation(db):
    assert wcs.resolve_owner(db, 1, 10)["name"] == "UsuarioDemo Escritorio Demo"
...
def test_contact_notes_crud_and_tenant(db):
    # contact id 1 belongs to org 1; user 10 (UsuarioDemo) is the author.
    n = wcs.add_note(db, 1, 1, 10, "  primeira nota  ")
    assert n["body"] == "primeira nota"                 # trimmed
    assert n["author_name"] == "UsuarioDemo Escritorio Demo"    # author resolved
